...
export class PrescriptionPopup extends AbstractAwaitablePopup {
    setup() {
        ...
        this.state = useState({ prescriptions: [], searchTerm: "", });

        onWillStart(async () => {
            const clinicId = this.pos.config.clinic_id?.[0];

            this.state.prescriptions = await this.orm.call(
    ...
    let records = this.state.prescriptions;
...
PrescriptionPopup.template = "PrescriptionPopupTemplate";

PrescriptionPopup.props = {
    close: Function,
    resolve: Function,
    zIndex: Number,
    id: Number,
