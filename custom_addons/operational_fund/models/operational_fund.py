...
class OperationalFundDisbursement(models.Model):
    ...
    def _onchange_therapist_data(self):
        """Safely extracts Therapist Data. Uses fallbacks to prevent crashes."""
        if self.therapist_ref_id:
            self.therapist_ved_number = getattr(self.therapist_ref_id, 'ved_number',
                                                getattr(self.therapist_ref_id, 'ved_no', ''))
            self.therapist_phone = getattr(self.therapist_ref_id, 'phone', getattr(self.therapist_ref_id, 'mobile',
                                                                                   getattr(self.therapist_ref_id,
                                                                                           'contact_number', '')))
    ...
    def _onchange_vendor_data(self):
        """Extracts Vendor Bank Data."""
        if self.vendor_ref_id:
            self.bank_account_name = self.vendor_ref_id.bank_account_name
            self.bank_account_number = self.vendor_ref_id.bank_account_number
            self.bank_ifsc_code = self.vendor_ref_id.bank_ifsc_code
...
class OperationalFundMarkPaidWizard(models.TransientModel):
    ...
    def action_confirm_paid(self):
        ...
        self.env['operational.fund.audit'].sudo().create({
            ...
            'amount': disb.amount,
            'reference': f'Direct Paid: {disb.name} - {disb.display_category}',
            'user_id': self.env.user.id
        })
