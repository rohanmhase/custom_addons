...
class ClinicTherapist(models.Model):
    ...
    # NEW: Direct toggle method for the dashboard gear icon
...
class ClinicScheduleAppointment(models.Model):
        ...
        for record in self:
            ...
            if self.search(domain):
                raise ValidationError(
                    _("Operational Conflict: This therapist is already booked or blocked during this hour!"))
    ...
    # NEW: Strict Gender Matching Compliance Rule
    ...
    def get_matrix_data(self, clinic_id, target_date, pulled_therapist_ids=None):
        ...
        for app in appointments_raw:
            ...
            formatted_appointments.append({
                ...
                'start_hour': local_time.hour if local_time else 0,
        ...
        enrollments = self.env['patient.enrollment'].search([
            ('clinic_id', '=', int(clinic_id)), ('payment_state', '=', 'paid'), ('remaining_sessions', '>', 0)
        ])
        ...
        all_therapists = self.env['clinic.therapist'].search([('active', '=', True)])
        assigned_staff = [t for t in all_therapists if
                          not t.allowed_branch_ids or int(clinic_id) in t.allowed_branch_ids.ids]

        return {
            'therapists': therapists, 'appointments': formatted_appointments,
            'clinics': clinics_records, 'regions': regions_records, 'selected_clinic_id': clinic_id,
            'kpis': {
                'rs_count': sum(1 for t in assigned_staff if t.designation == 'rs'),
                'fixed_count': sum(1 for t in assigned_staff if t.designation == 'fixed'),
                'floater_count': sum(1 for t in assigned_staff if t.designation in ['floater', 'hv']),
                'utilization': round((len(appointments_raw.filtered(lambda a: a.therapist_id)) / (
                            len(board_therapists) * 15)) * 100) if len(
                    board_therapists) > 0 else 0,
    ...
    def get_daily_analytics(self, target_date):
        ...
        active_enrollments = self.env['patient.enrollment'].search(
            [('payment_state', '=', 'paid'), ('remaining_sessions', '>', 0)])
