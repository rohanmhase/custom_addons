...
export class ClinicMatrixDashboard extends Component {
    setup() {
        ...
        this.state = useState({
            ...
            lastFetchedClinic: 0,
            pulledTherapistIds: [], 
            hours: Array.from({ length: 15 }, (_, i) => i + 7),

            therapists: [], appointments: [], clinics: [], regions: [], rosterData: [],
            ...
            attendanceLedger: [], attendanceSearchQuery: "", expandedRows: [],
            selectedAppointment: null, isActionModalOpen: false, quickReassignTherapist: 0,
            isAllotModalOpen: false, allotableTherapists: [], allotSearchQuery: "", selectedTherapistObj: null,
            analyticsData: null, isDrillDownModalOpen: false, drillDownTitle: "", drillDownRecords: [], drillDownType: "",
            isTherapistActionModalOpen: false, selectedTherapistForAction: null,
            isLateModalOpen: false, lateExpectedHour: "10",
            isSmartViewOpen: false, smartViewData: null, smartViewClinicName: ""
        });
    ...
    // UPDATED: Now receives the patient's gender to enforce strict compliance mapping
    getFreeTherapistsForHour(hour, patientGender = false) {
        ...
        return this.state.therapists.filter(t => {
            ...
            const hasSlot = this.state.appointments.some(a => a.therapist_id === t.id && a.start_hour === hour);
            return !hasSlot;
        });
    }

    addExtraHour() {
        const lastHour = this.state.hours[this.state.hours.length - 1];
        if (lastHour < 23) { this.state.hours.push(lastHour + 1); }
    }

    async switchTab(tabName) {
        this.state.activeTab = tabName;
        if (tabName === "roster") { await this.loadRosterMetadata(); }
        else if (tabName === "attendance") { await this.loadAttendanceLedger(); }
        else if (tabName === "analytics") { await this.loadAnalyticsData(); }
        else { await this.refreshGrid(); }
    }

    async onRegionChange() {
        const availableClinics = this.filteredClinics;
        if (availableClinics.length > 0) { this.state.selectedClinic = availableClinics[0].id; }
        else { this.state.selectedClinic = 0; }
        await this.refreshGrid();
    }
    ...
    async loadRosterMetadata() { this.state.rosterData = await this.orm.call("clinic.schedule.appointment", "get_roster_data", [this.state.selectedDate]); }
    ...
    async loadAnalyticsData() { this.state.analyticsData = await this.orm.call("clinic.schedule.appointment", "get_daily_analytics", [this.state.selectedDate]) || null; }
    ...
    closeSmartView() { this.state.isSmartViewOpen = false; this.state.smartViewData = null; }
    ...
    closeDrillDown() { this.state.isDrillDownModalOpen = false; this.state.drillDownRecords = []; }

    toggleRow(therapistId) {
        if (this.state.expandedRows.includes(therapistId)) { this.state.expandedRows = this.state.expandedRows.filter(id => id !== therapistId); }
        else { this.state.expandedRows.push(therapistId); }
    }

    getSlotData(therapistId, hour) {
        const slots = this.state.appointments.filter(app => app.therapist_id === therapistId && app.start_hour === hour);
    ...
    getUtcDateTimeString(hour) {
        let [year, month, day] = this.state.selectedDate.split('-');
        let localDate = new Date(year, month - 1, day, hour, 0, 0);
    ...
    async onCreateNewTherapistClick() {
        ...
        }, { onClose: async () => { await this.refreshGrid(); await this.loadRosterMetadata(); } });
    }
    ...
    selectTherapistToAllot(therapist) { this.state.selectedTherapistObj = therapist; }
    ...
    // NEW: Function to instantly toggle a therapist as a Buffer/Emergency walk-in runway
    ...
    async onAddTimeClick(hour) {
        let startDt = `${this.state.selectedDate} ${hour.toString().padStart(2, '0')}:00:00`;
        this.actionService.doAction({
            type: "ir.actions.act_window",
            res_model: "clinic.schedule.appointment",
            views: [[false, "form"]],
    ...
    async onSlotClick(therapistId, hour) {
        const existing = this.getSlotData(therapistId, hour);
        if (existing) {
            this.state.selectedAppointment = existing;

            // Set dropdown back to top safely matching strict gender parameters
            const freeStaff = this.getFreeTherapistsForHour(hour, existing.patient_raw_gender);
            ...
            let startDt = this.getUtcDateTimeString(hour);
            this.actionService.doAction({
                type: "ir.actions.act_window",
                res_model: "clinic.schedule.appointment",
                views: [[false, "form"]],
